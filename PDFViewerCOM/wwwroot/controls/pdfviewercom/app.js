...
class PDFViewer {
    constructor() {
        ...
        // Select/move state (drawing mode)
        this.selectedPageNum = null;
        this.selectedStrokeIdx = -1;
        this.isDraggingStroke = false;
        this.dragLastX = 0;
        this.dragLastY = 0;
        // Passive select/move state (no drawing mode required)
        this._passiveDragging = false;
        this._passiveDragX = 0;
        this._passiveDragY = 0;
    ...
    async loadBase64(b64, loadType) {
        this.showLoading();
        this._loadType = loadType || 'base64';
        this._pdfUrl = null;
        this._pdfFilename = 'document.pdf';
        try {
            const bin = atob(b64);
    ...
    async documentLoaded() {
        ...
        this.welcomeMessage.classList.add('hidden');
        this.errorContainer.classList.add('hidden');
        // Clear previous document state — prevents bleeding of drawings/highlights into new load.
        // clearAnnotations() wipes both the state arrays AND any DOM nodes that live outside
        // the viewer subtree (which `renderAllPages`' innerHTML='' would otherwise miss).
        ...
        // Fire AFTER all page DOMs exist — Clarion's ImportAnnotationsData needs page-N elements
        // present when placeNote runs, otherwise notes for pages not yet rendered are dropped.
        this.sendMessage('documentLoaded', { pageCount: this.pageCount, title, loadType: this._loadType });
    }
    ...
    async renderAllPages() {
        ...
        for (let n = 1; n <= this.pageCount; n++) {
            ...
            // Passive select/move - works without entering drawing mode (capture phase so we
            // intercept the event before text-layer / annotation-layer children receive it).
            const pageNum = n;
            pageDiv.addEventListener('mousemove', (e) => {
                if (this.drawingEnabled) return;
                const dc = pageDiv.querySelector('.drawing-canvas');
                if (!dc) return;
                const r = dc.getBoundingClientRect();
                const x = (e.clientX - r.left) * (dc.width / r.width);
                const y = (e.clientY - r.top)  * (dc.height / r.height);
                if (this._passiveDragging && this.selectedStrokeIdx >= 0 && (e.buttons & 1)) {
                    const dx = x - this._passiveDragX, dy = y - this._passiveDragY;
                    this._passiveDragX = x; this._passiveDragY = y;
                    const hist = this.drawingHistory[pageNum];
                    if (hist && hist[this.selectedStrokeIdx]) {
                        this._moveStroke(hist[this.selectedStrokeIdx], dx, dy, dc.width, dc.height);
                        this.redrawPageCanvas(pageNum, dc);
                        e.stopPropagation();
                    }
                    return;
                }
                const hist = this.drawingHistory[pageNum] || [];
                pageDiv.style.cursor = hist.some(s => this._hitTestStroke(s, x, y, dc.width, dc.height)) ? 'move' : '';
            }, true);
            ...
            pageDiv.addEventListener('mousedown', (e) => {
                ...
                for (let i = hist.length - 1; i >= 0; i--) {
                    if (this._hitTestStroke(hist[i], x, y, dc.width, dc.height)) { foundIdx = i; break; }
                }
                if (foundIdx >= 0) {
                    if (this.selectedPageNum !== null && this.selectedPageNum !== pageNum) {
                        this.redrawPageCanvas(this.selectedPageNum, null);
                    }
                    this.selectedStrokeIdx = foundIdx;
                    this.selectedPageNum = pageNum;
                    this._passiveDragging = true;
                    this._passiveDragX = x; this._passiveDragY = y;
                    this.redrawPageCanvas(pageNum, dc);
                    e.stopPropagation();
                } else {
                    if (this.selectedStrokeIdx >= 0) {
                        this.selectedStrokeIdx = -1;
                        this.redrawPageCanvas(this.selectedPageNum, null);
                    }
                    this._passiveDragging = false;
            ...
            pageDiv.addEventListener('mouseup',    () => { this._passiveDragging = false; }, true);
            pageDiv.addEventListener('mouseleave', () => { this._passiveDragging = false; }, true);

            // Right-click on a stroke -> delete it (works without entering drawing mode).
            pageDiv.addEventListener('contextmenu', (e) => {
                ...
                const hist = this.drawingHistory[pageNum] || [];
                for (let i = hist.length - 1; i >= 0; i--) {
                    if (this._hitTestStroke(hist[i], x, y, dc.width, dc.height)) {
                        hist.splice(i, 1);
                        const idx = this.annotations.findIndex(a => a.type === 'drawing' && a.page === pageNum);
                        if (idx >= 0) this.annotations.splice(idx, 1);
                        if (this.selectedPageNum === pageNum && this.selectedStrokeIdx === i) {
                            this.selectedStrokeIdx = -1;
                            this.selectedPageNum = null;
                        }
                        this.redrawPageCanvas(pageNum, dc);
    ...
    async rerenderAllPages() {
        ...
        this.reapplyHighlights();
        // Re-position note icons (same fractional scheme)
        this.reapplyNotes();
    ...
    // Re-position note icons after zoom/rotation change — fractional coords * new page size
    reapplyNotes() {
        this.annotations.filter(a => a.type === 'note' && a._el).forEach(a => {
            const pageEl = document.getElementById(`page-${a.page}`);
            if (!pageEl) return;
            a._el.style.left = `${a.fx * pageEl.offsetWidth}px`;
            a._el.style.top  = `${a.fy * pageEl.offsetHeight - 12}px`;
        });
    }
    ...
    showNoteInput(pageNum, x, y) {
        ...
        ok.onclick = () => {
            ...
            if (t) this.placeNote(pageNum, x / pageEl.offsetWidth, y / pageEl.offsetHeight, t);
            popup.remove();
        };
    ...
    placeNote(pageNum, fx, fy, text) {
        ...
        noteEl.style.left = `${fx * pw}px`;
        noteEl.style.top  = `${fy * ph - 12}px`;
        ...
        // Define annot early so event handlers can update it
        const annot = { type: 'note', page: pageNum, fx, fy, text, _el: noteEl };
        ...
        // Double-click → edit note text (same popup style as create)
        noteEl.addEventListener('dblclick', (e) => {
            ...
            popup.id = 'note-popup';
            popup.className = 'note-popup';
            popup.style.left = `${parseFloat(noteEl.style.left)}px`;
            popup.style.top  = `${parseFloat(noteEl.style.top) + 28}px`;
            ...
            save.onclick = () => {
                const t = ta.value.trim();
                if (t) { annot.text = t; tooltip.textContent = t; }
                popup.remove();
            };
        ...
        const onMove = (e) => {
            ...
            const newLeft = origLeft + dx, newTop = origTop + dy;
            noteEl.style.left = `${newLeft}px`;
            noteEl.style.top  = `${newTop}px`;
            // Keep fractional coords in sync so save/reload and zoom preserve position
            annot.fx = newLeft / pageEl.offsetWidth;
            annot.fy = (newTop + 12) / pageEl.offsetHeight;
        };
    ...
    startDrawing(e, canvas) {
        ...
        if (this.drawMode === 'select') {
            ...
            for (let i = history.length - 1; i >= 0; i--) {
                if (this._hitTestStroke(history[i], x, y, canvas.width, canvas.height)) { foundIdx = i; break; }
            }
        ...
        // Store all coordinates as fractions of canvas dimensions so strokes rescale with zoom.
        const cw = canvas.width, ch = canvas.height;
        const fx = x / cw, fy = y / ch;
        const fwidth = this.strokeWidth / cw;  // line width also fractional so it scales

        if (this.drawMode === 'freehand') {
            this.currentStroke = { type: 'freehand', strokeColor: this.strokeColor, fwidth, points: [{ fx, fy }] };
        } else {
            this.currentStroke = { type: this.drawMode, strokeColor: this.strokeColor, fillColor: this.fillColor, fwidth, fx1: fx, fy1: fy, fx2: fx, fy2: fy };
        }
    }
    ...
    continueDrawing(e, canvas) {
        ...
        if (this.drawMode === 'select') {
            ...
            if (history && history[this.selectedStrokeIdx]) {
                this._moveStroke(history[this.selectedStrokeIdx], dx, dy, canvas.width, canvas.height);
        ...
        if (this.drawMode === 'freehand') {
            ...
            ctx.lineTo(x, y);
            ctx.stroke();
            this.currentStroke.points.push({ fx: x / cw, fy: y / ch });
            this.lastX = x; this.lastY = y;
        } else {
            this.currentStroke.fx2 = x / cw;
            this.currentStroke.fy2 = y / ch;
            if (!this._rafPending) {
    ...
    _renderStroke(ctx, stroke) {
        ...
        ctx.lineWidth   = (stroke.fwidth != null ? stroke.fwidth * cw : (stroke.width || 2));
        ctx.lineCap = 'round'; ctx.lineJoin = 'round';

        if (stroke.type === 'freehand') {
            if (!stroke.points?.length) { ctx.restore(); return; }
            ctx.beginPath();
            ctx.moveTo(stroke.points[0].fx * cw, stroke.points[0].fy * ch);
            stroke.points.forEach(p => ctx.lineTo(p.fx * cw, p.fy * ch));
            ctx.stroke();
        } else if (stroke.type === 'rect') {
            const x1 = stroke.fx1 * cw, y1 = stroke.fy1 * ch;
            const x2 = stroke.fx2 * cw, y2 = stroke.fy2 * ch;
    ...
    _hitTestStroke(stroke, x, y, canvasW, canvasH) {
        const lineW = (stroke.fwidth != null ? stroke.fwidth * canvasW : (stroke.width || 2));
        const pad = Math.max(8, lineW / 2 + 4);
        if (stroke.type === 'freehand') {
            for (const p of stroke.points || []) {
                const px = p.fx * canvasW, py = p.fy * canvasH;
                if (Math.abs(px - x) <= pad && Math.abs(py - y) <= pad) return true;
            }
            return false;
        }
        // rect / circle — test bounding box
        const x1 = stroke.fx1 * canvasW, y1 = stroke.fy1 * canvasH;
        const x2 = stroke.fx2 * canvasW, y2 = stroke.fy2 * canvasH;
    ...
    _moveStroke(stroke, dx, dy, canvasW, canvasH) {
        const fdx = dx / canvasW, fdy = dy / canvasH;
        if (stroke.type === 'freehand') {
            stroke.points.forEach(p => { p.fx += fdx; p.fy += fdy; });
        } else {
            stroke.fx1 += fdx; stroke.fy1 += fdy;
            stroke.fx2 += fdx; stroke.fy2 += fdy;
        }
    }
    ...
    _drawSelectionBox(ctx, stroke) {
        ...
        if (stroke.type === 'freehand') {
            const xs = stroke.points.map(p => p.fx * cw);
            const ys = stroke.points.map(p => p.fy * ch);
    ...
    importAnnotations(json) {
        try {
            ...
            annotations.filter(a => a.type === 'note').forEach(a => {
                this.placeNote(a.page, a.fx, a.fy, a.text);
            });
